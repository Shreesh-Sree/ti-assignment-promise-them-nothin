...
**Coordination:** static per-node partition of the global limit, with an optional two-phase shrink-before-grow rebalancer behind the same `Coordinator` interface. The static coordinator fixes each node's share at `ceil(quota/N)`, so the sum of shares can be up to `N-1` above the raw quota for non-evenly-dividing limits (e.g. `ceil(100/3)×3 = 102 > 100`); no coordination invariant is needed because shares never change. The optional peer coordinator's rebalancer maintains a relative invariant: `sum(shares) ≤ quota` throughout every rebalance round, because the shrink-before-grow ordering guarantees decreases are confirmed before any corresponding increase is sent — so over-admission from a mid-round coordination split cannot occur. The system-wide admission bound `(ceil(quota/N) + burst) × N` holds for both coordinators via each node's GCRA spacing guarantee and applies regardless of whether shares are static or rebalancing. Full proof in `DESIGN-NOTES.md` Part 2, verified against real captured timestamps in Parts 3–4.
